...
import {
  getContractAddress,
  APRIL_MERKLE_DISTRIBUTION,
} from "@deployments/utils";

const expect = getWaffleExpect();

describe("RewardsApr21MerkleDistributor", () => {
  ...
  before(async () => {
    ...
    const deployedMerkleDistributorContract = await getContractAddress("RewardsApr21MerkleDistributor");
    distributorContractInstance = new MerkleDistributor__factory(deployer.wallet).attach(deployedMerkleDistributorContract);
  });

  addSnapshotBeforeRestoreAfterEach();

  describe("RewardsApr21MerkleDistributor", async () => {
    ...
    it("should have the correct unclaimed", async () => {
      for (let rootIndex = 0; rootIndex < APRIL_MERKLE_DISTRIBUTION.length; rootIndex++) {
