...
describe("StakingRewardsV2 - MVI", () => {
  ...
  before(async () => {
    ...
    const rewardsDeploy  = await getContractAddress("StakingRewardsV2 - MVI");
    stakingRewards = new StakingRewardsV2__factory(deployer.wallet).attach(rewardsDeploy);
  });

  addSnapshotBeforeRestoreAfterEach();

  describe("StakingRewardsV2 - MVI", async () => {
    ...
    it("should have the correct staking token", async () => {
      ...
      expect(stakingToken).to.eq(await findDependency("MVI_UNI_POOL"));
    });
